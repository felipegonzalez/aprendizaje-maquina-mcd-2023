---
title: "Concurso: xgboost afinado"
format: html
---
...
params <- parameters(learn_rate(range = c(-2-5, -0.5), log10_trans()),
                     mtry(range = c(5,9)),
                     tree_depth(range = c(3, 6)),
                     trees(range = c(5000, 15000)))

lambda_grid <- grid_regular(params, levels = c(learn_rate = 10, mtry = 5, tree_depth = 4, trees = 3)) 
...
```{r}
write_csv(submission, "entregas/modelo-afinado-xgboost.csv")
```
